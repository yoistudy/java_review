...
```
- 캡슐화 : Data와 Data조작을 연산을 하나로 묶는 작업
- 추상화 : 각 클래스의 공통적 성질을 추출하여 상위 클래스에서 사용
- 다형성 : 다양한 형태/성질로 사용할 수 있는 방법
  ex) System클래스의 println 매소드
...
Pernament를 제외한 Old, Young 영역에 저장되는 인스턴스의 경우 가비지컬렉팅의 주요한 대상이 됨.
Pernament의 경우 코드상에서 참조가 이뤄지는 인스턴스이기 떄문에, 가비지컬렉팅 대상에서 빠짐
```
#### Call Stack
